...
| PI Point Type Change Detected | Check the [PI to OCS event logs](xref:view-logs) in the Windows Event Viewer for the cause of the error. Filter the logs by "Type Change" or event ID "204" to locate the error message that contains the type changed PI point id. Change the type of the point id to match it's respective SDS Stream type and restart the transfer to allow data to resume transferring from that PI point. See [PI point type change](xref:pi-point-change) for more information. |  
